...
const { connectWallet, fetchAll, getMktBalance, getItemsSold, getCount } = require("./lib/ethers").init(window.ethereum);

const Navigation = ({ links, isActive, updateActive }) => {
    return(
    <div className="d-flex flex-column">
        <button type="button" className="align-self-end btn btn-dark mt-3 mx-5" data-bs-toggle="modal" data-bs-target="#modal">Mint NFT+</button>
...
export default function Marketplace() {
    ...
    const getNFTs = async () => {
        ...
        const totalNft = await getCount()
        setTotalNFTs(`${totalNft}`)
    ...
    useEffect(() => {
        connectWallet()
            .then(getMktBalance)
            .then(getItemsSold)
            .then(({ itemsSold, contractBalance, account, totalNFTs}) => {
                setAccount(account)
    ...
    return (
      <>
        <Navbar account={account} balance={mktBalance} itemsSold={itemsSold} nftCount={totalNFTs} />
